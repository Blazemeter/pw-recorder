...
import { open } from 'pw-recorder-core/lib/utilsBundle';
import { MultiMap, getPackageManagerExecCommand } from 'pw-recorder-core/lib/utils';
import fs from 'fs';
...
import { HttpServer, assert, calculateSha1, copyFileAndMakeWritable, gracefullyProcessExitDoNotHang, removeFolders, sanitizeForFilePath, toPosixPath } from 'pw-recorder-core/lib/utils';
...
class HtmlBuilder {
  ...
  async build(metadata: Metadata, projectSuites: Suite[], result: FullResult, topLevelErrors: TestError[]): Promise<{ ok: boolean, singleTestId: string | undefined }> {
    ...
    // Copy app.
    const appFolder = path.join(require.resolve('pw-recorder-core'), '..', 'lib', 'vite', 'htmlReport');
    await copyFileAndMakeWritable(path.join(appFolder, 'index.html'), path.join(this._reportFolder, 'index.html'));

    // Copy trace viewer.
    if (this._hasTraces) {
      const traceViewerFolder = path.join(require.resolve('pw-recorder-core'), '..', 'lib', 'vite', 'traceViewer');
